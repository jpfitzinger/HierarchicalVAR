...
#' @param p.value Prune hierarchical tree by ignoring branches with an insignificant parent coefficient (specify significance level)
                ...
                )
{
  ...
  for (i in 1:K) {
    y_i <- yend[,i]
    coefs[,i] <- getRecBipart(y_i, datamat, exog = rhs, cov.mat, cor.mat, ord$order, ord$height, distance.lambda,
                              maxnsplits = max.splits, stop.rule = min.cluster, p.crit = p.value, det = det)
  }
